...
config2 = {
        ...
        "petri1": (
            "0gy",
            {
                "souris1": {
                    ...
                    "echantillon2": ["zone1", "zone2", "zone3"],
                },
                "souris2": {
                    "echantillon1": ["zone1", "zone2", "zone3"],
                    "echantillon2": ["zone1", "zone2", "zone3"],
    ...
    "jour2": {
        ...
        "petri2": (
            "45gy",
            {"souris1": ["zone1", "zone2"], "souris2": ["zone1", "zone2", "zone3"]},
        ),
        "petri3": (
            "45gy + P",
            {
                "souris1": ["zone1", "zone2", "zone3"],
...
def add_additional_experimental_info(dataframe, name="surya-dataset-description"):
    ...
    for batch, petris in config1.items():
        for petri, (echantillon, dose, type_) in petris.items():
            ...
            masque = (
                (dataframe["exp"].isin([2, 3]))
                & (dataframe["batch"] == num_batch)
...
def fix_acquisition_errors(df, name="surya-dataset-description"):
    ...
    def renumber_sequentially_in_time(df, mask):
        ...
        if len(set(list_rows)) == len(list_rows):
            raise ValueError(
                f"Les 'indice1' sont uniques: rien a renumeroter {list_rows}"
            )
    ...
    df = df.drop(
        index="exp_1/jour8/frais/Raman/petri2/petri2_souris3_zone1/20260519_jour6_raman_petri2_souris3_45Gy_zone1_RamanShift__0__11-41-01-478.txt",
        errors="ignore",
    )
...
class TestSuryaDataset(unittest.TestCase):
    ...
    def test_002_the_corrections_leave_a_usable_table(self):
        ...
        self.files.initialize()
        self.files.finalize(
            [add_additional_experimental_info, fix_acquisition_errors, delete_test_data]
        )
        df = self.files.dataframe
