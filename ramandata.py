...
class RamanData:
    ...
    SPECTRUM_LINE = re.compile(
        rb"^[ \t]*(\d+[.,]?\d*)[ \t,;]+(-?\d*[.,]?\d*)[ \t]*\r?$", re.MULTILINE
    )
    ...
    def extract_header_from_file(root, relative_path):
        ...
        try:
            ...
            print(
                f"Warning: {file_path} is not recognized (probably accented characters)"
            )
    ...
    def initialize(self, mask=None, verbose=True):
        ...
        spectra = self.files.read_data_files(
            reader_method=self.read_spectrum, mask=mask
        )
        ...
        rejected = [
            i for n, group in lengths.items() if n != expected_length for i in group
        ]
        ...
        for index in usable:
            if not np.array_equal(spectra[index][0], axis):
                raise ValueError(
                    f"{index} has the same number of points but not the same "
        ...
        assert self.X.shape[0] == len(
            self.meta
        ), "X and meta must describe the same spectra"
    ...
    def report(self, verbose=True, show=10):
        ...
        counts = {
            "offered": self.files_offered,
            "kept": kept,
            "rejected": len(self.rejected),
            "points": self.expected_length,
            "lengths": lengths,
        }
        ...
        print(
            f"{counts['offered']} files offered, {counts['kept']} spectra of "
    ...
    def select(self, mask=None, **criteria):
        ...
        if mask is not None:
            if isinstance(mask, pd.Series):
                ...
                if mask.shape != (len(self.meta),):
                    raise ValueError(
                        f"The mask holds {mask.size} entries, but there are "
        ...
        if unknown:
            raise ValueError(
                f"No such column: {unknown}. A criterion on a column that does "
        ...
        if not chosen.any():
            asked = dict(criteria)
            raise ValueError(
                f"Nothing matches {asked if asked else 'the given mask'}: the "
    ...
    def averaged(self, ignore=(), on=None, verbose=True):
        ...
        if on is not None and len(ignore) > 0:
            raise ValueError(
                "Give either `on` or `ignore`, not both: each is the other's "
        ...
        if unknown:
            raise ValueError(
                f"No such column: {unknown}. A misspelled name would quietly "
        ...
        X = np.vstack(
            [self.X[codes == group].mean(axis=0) for group in range(n_groups)]
        )
        summary["n_averaged"] = np.bincount(codes, minlength=n_groups)

        labels = [
            " ".join(f"{column}={row[column]}" for column in fingerprint) or "all"
        ...
        if not summary.index.is_unique:
            raise ValueError(
                "Two different groups end up with the same fingerprint once it is "
        ...
        assert result.X.shape[0] == len(
            result.meta
        ), "X and meta must describe the same rows"

        if verbose:
            alone = int((summary["n_averaged"] == 1).sum())
            largest = int(summary["n_averaged"].max())
            print(
                f"{len(self)} spectra averaged into {n_groups} rows "
            ...
            if alone > n_groups / 2:
                ...
                for column in fingerprint:
                    ...
                    if not rest:
                        continue
                    without = (
                        int(meta.groupby(rest, dropna=False, sort=False).ngroup().max())
                        + 1
                    )
                    if without < n_groups:
                ...
                if culprits:
                    print(
                        f"    most rows came out alone. Leaving out any of {culprits} "
    ...
    def read(cls, path):
        ...
        if list(stored["files"]) != list(meta.index):
            raise ValueError(
                "The matrix and the metadata no longer describe the same spectra"
            )
...
class TestRamanData(unittest.TestCase):
    ...
    def test_010_read_one_spectrum(self):
        path = write_spectrum_file(
            self.root / "one.txt", points=4, first=100.0, step=10.0
        )
    ...
    def test_011_header_is_not_taken_for_data(self):
        ...
        np.testing.assert_array_equal(
            RamanData.read_spectrum(with_header)[1], RamanData.read_spectrum(without)[1]
        )
    ...
    def test_012_comma_as_decimal_mark(self):
        ...
        np.testing.assert_allclose(
            RamanData.read_spectrum(french)[0], RamanData.read_spectrum(english)[0]
        )
    ...
    def test_014_negative_intensities_are_kept(self):
        ...
        path = write_spectrum_file(
            self.root / "negative.txt", points=4, intensity=lambda i: -5.0 - i
        )
    ...
    def test_025_a_spectrum_of_another_length_is_left_out(self):
        ...
        write_spectrum_file(
            ...
            / "sample1"
            / "zone1"
            / "sample1_dose0_zone1_other_instrument.txt",
    ...
    def test_027_two_different_axes_stop_everything(self):
        ...
        """
        write_spectrum_file(
            self.root / "sample1" / "zone1" / "sample1_dose0_zone1_other_axis.txt",
            points=self.POINTS,
            first=999.0,
        )
        self.datafiles.initialize()
    ...
    def test_029_report_names_what_was_left_out(self):
        ...
        write_spectrum_file(
            folder / "sample1_dose0_zone1_other_instrument.txt", points=self.POINTS * 2
        )
        ...
        self.assertEqual(
            sorted(Path(name).name for name in raman.rejected),
    ...
    def test_02b_report_survives_being_written_out(self):
        ...
        folder = self.root / "sample1" / "zone1"
        (folder / "note about sample1.txt").write_text(
            "pas un spectre\n", encoding="latin-1"
        )
    ...
    def test_053_selecting_the_rows_where_a_column_is_empty(self):
        ...
        """
        write_spectrum_file(
            self.root / "sample1_dose0_elsewhere.txt", points=self.POINTS
        )
        raman = self.initialized()
        ...
        self.assertEqual(
            len(raman.select(zone=[1, 2, None])), self.expected_spectra + 1
        )
    ...
    def test_037_not_one_spectrum_is_lost(self):
        ...
        self.assertEqual(
            int(mean.meta.loc[mean.meta["zone"].isna(), "n_averaged"].iloc[0]), 1
        )
    ...
    def test_03j_a_true_or_false_column_the_group_disagrees_on(self):
        ...
        for number in range(4):
            ...
            write_spectrum_file(
                folder / f"sample1_zone1_{word}{number}.txt", points=self.POINTS
            )

        files = DataFiles(
            folder, metadata_patterns=self.PATTERNS + [r"(?P<is_dark>dark)"]
        )
